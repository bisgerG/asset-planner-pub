 ...
 * 它只對「已發生的價格」反應，不預測。
 *
 * 回測（backtest/pe_backtest.py，2000-07~2026-07，26.0 日曆年；CAGR 分母為交易日/252＝24.71）：
 *   準備金 70/30  平均曝險 168%  CAGR 18.3%  MDD -76.7%  Calmar 0.238  最長水下 3.6 年
 *   固定 1.5x     平均曝險 150%  CAGR 15.3%  MDD -74.7%  Calmar 0.205  最長水下 6.3 年
 ...
 *   1995-2026、每個 sleeve 內部皆 70/30：100% 台股 Calmar 0.170、波動 36.19%、
 *   15 年最差 1.48x  →  台股40/SPX60 為 0.203、23.14%、2.70x。
 * 更重要的是模型風險：把台股正二的逆價差紅利歸零（α −0.14% → −5.11%/年），
 * 單押台股 Calmar 掉三成（0.170 → 0.116）、15 年最差世代虧錢（0.76x），混合仍有 0.180 / 1.97x。
 *
 * ── V2.4：那斯達克（QLD）下架 ─────────────────────────────────────────
...
const STRATEGY = {
  ...
  research: {
    ...
    products2x: {
      ...
      // ⚠ 回測合成正二用的 α=−0.14%/年是錯的。它的來源檔（Strategy repo 的
      //    etf_00631L.csv）少了 17 個交易日，其中整個 2016-02（13 天）不見了，
      //    害 2016-03-01 變成一個假的單日大漲，把全期 α 從 −0.83% 拉到 −0.14%。
      //    這是 README 早就記載過的同一個 bug（缺口後第一天的日漲跌是假的），
      //    只是沒人往 ETF 來源檔查。本 repo 自抓的完整序列已修正。
      alphaBug: { published: -0.14, corrected: -0.83, missingDays: 17,
                  gapFrom: '2016-02-01', gapTo: '2016-02-26', final26Inflation: 19.9 },
    },
...
const BLEND = {
  ...
  rows: [
    { name: '100% 台股正二',            tw: 100, spx: 0,   cagr: 14.2, vol: 36.19, mdd: -83.8, calmar: 0.170, uw: 7.2,  kelly: 73, med15: 5.55, worst15: 1.48, cur: true },
    { name: '100% S&P500 2x',          tw: 0,   spx: 100, cagr: 13.1, vol: 27.65, mdd: -73.0, calmar: 0.180, uw: 6.4,  kelly: 54, med15: 5.10, worst15: 2.31 },
    { name: '100% 那斯達克 2x（對照，非選項）', tw: 0, spx: 0, ndx: 100, cagr: 13.9, vol: 41.02, mdd: -96.6, calmar: 0.144, uw: 20.6, kelly: 87, med15: 8.89, worst15: 0.40, ref: true },
    { name: '台股50 / SPX50',           tw: 50,  spx: 50,  cagr: 15.2, vol: 23.88, mdd: -75.1, calmar: 0.202, uw: 6.2,  kelly: 38, med15: 6.17, worst15: 2.62 },
    { name: '台股40 / SPX60',           tw: 40,  spx: 60,  cagr: 15.0, vol: 23.14, mdd: -74.1, calmar: 0.203, uw: 6.0,  kelly: 36, med15: 6.06, worst15: 2.70, best: true },
  ],
  ...
  stress: {
    note: '把台股正二的逆價差紅利歸零（純 α −0.14% → −5.11%/年）之後重跑',
    rows: [
      { name: '100% 台股正二',          calmar: 0.116, uw: 10.0, med15: 2.87, worst15: 0.76, loss: 3, cur: true },
      { name: '混合（以 台股40/SPX50/NDX10 為代表）', calmar: 0.180, uw: 7.4, med15: 4.85, worst15: 1.97, loss: 0, proxy: true },
    ],
  },
...
// ⚠ 首頁主回測表的 Calmar 0.238 是 2000-07 起算；這張表誠實揭露起點選擇的貢獻。
const BACKTEST_COMMON = {
  span: '1995-01 ~ 2026-07（31.6 年，含 1997 亞洲金融風暴）',
  rows: [
    { name: '台股正二 準備金 70/30',        expo: 171, cagr: 14.2, mdd: -83.8, calmar: 0.170, uw: 7.2,  cur: true },
    { name: '台股正二（紅利反轉）',          expo: 170, cagr: 9.8,  mdd: -84.6, calmar: 0.116, uw: 10.0 },
    { name: 'S&P500 2x 準備金 70/30',      expo: 153, cagr: 13.5, mdd: -74.0, calmar: 0.182, uw: 6.4 },
    { name: 'S&P500 3x 準備金 70/30',      expo: 225, cagr: 16.3, mdd: -87.7, calmar: 0.186, uw: 6.9 },
...
(async () => {
  ...
  const payload = {
    ...
    strategy: {
      ...
      // ⚠ 年數口徑：pe_backtest.py 的 n_yr = 交易日 / 252，2000-07-03~2026-07-28 共 6,227 個交易日
      //   → 24.71「年」，但日曆區間是 26.0 年。用較小的分母會把 CAGR 撐高（63.07x 在 24.71 年是
      //   18.3%、在 26.0 年是 17.2%）。標籤一律寫日曆年，CAGR 分母的差異由頁面 tblnote 揭露。
      backtest: {
        span: '2000-07 ~ 2026-07（26.0 年）',
        // ⚠ cur（本策略）而不是 best（最佳）—— 70/30 的 Calmar 0.238 並不是本表最高的，
        //   50/50 的 0.246 與 60/40 的 0.243 都比它高。選 70/30 的理由是終值（63.07x vs 52.95x，
        //   高 19%）而最大回撤只差 5.7pp，且 3.6 年是最長水下「尚未跳升」的最後一格。
        //   README 原表有 50/50 與 80/20 兩列，這裡一併列出，不要只留對本策略有利的比較對象。
        rows: [
          { name: '準備金 70/30（本策略）',   expo: 168, fin: 63.07, cagr: 18.3, mdd: -76.7, calmar: 0.238, uw: 3.6, cur: true },
          { name: '準備金 80/20（更積極）',   expo: 178, fin: 65.74, cagr: 18.5, mdd: -79.6, calmar: 0.232, uw: 5.7 },
          { name: '準備金 60/40（較保守）',   expo: 158, fin: 58.56, cagr: 17.9, mdd: -73.8, calmar: 0.243, uw: 3.6 },
          { name: '準備金 50/50（最保守）',   expo: 149, fin: 52.95, cagr: 17.4, mdd: -71.0, calmar: 0.246, uw: 2.9 },
          { name: '固定 2.0x',              expo: 200, fin: 62.50, cagr: 18.2, mdd: -85.3, calmar: 0.214, uw: 6.8 },
          { name: '固定 1.5x',              expo: 150, fin: 33.60, cagr: 15.3, mdd: -74.7, calmar: 0.205, uw: 6.3 },
          { name: 'P/E 定槓桿（校正同曝險）',    expo: 150, fin: 23.46, cagr: 13.6, mdd: -75.6, calmar: 0.18,  uw: 6.7 },
        ],
      },
