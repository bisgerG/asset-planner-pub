...
| `/start/` | 🚀 快速開始精靈 | 九個白話問題 → **九階風險階梯**自己選一階,幫沒經驗的人配好第一版計畫。趕時間可走**快速通道**:四題採用畫面上標「不知道就選這個」的答案並跳過(10 屏 → 6 屏),完成頁會照實列出是哪四題 |
| `/plan-result/` | ★ 一頁式結果 | **你的計畫長什麼樣、撐不撐得住、接下來做什麼**;含**停投階梯** —— 實際模擬「提早幾歲停止投入」各要付多少代價 |
...
| `assets/shell.js` | 頂部導覽、進度指示、進階展開器、單一份名詞辭典(75 條)＋ `data-tip` 就地氣泡 |
...
設計文件(`docs/IA.md`)留在開發 repo,但下面三條在這裡踩到會**無聲壞掉**,先寫在這:

1. **名詞解釋有兩套機制,別混用。** `.term` 由 `shell.js` 的 `annotateTerms()` 自動掃,
   查的是那份 75 條的辭典、**每個詞每頁只標第一次出現**;`.tip` + `data-tip="一句話"`
   是手動掛的就地說明,給**數字**用。兩者外觀相同、共用同一顆氣泡。
   加了新的重繪容器要記得補進 `DYN_IDS`,否則 `innerHTML` 會沖掉 `.term` 按鈕留下壞標記。
2. **`/report/` 是 A4 列印文件,不准用 `data-tip`,也不放 `<details>`。**
   紙上沒有滑鼠也沒得展開 —— 藏進氣泡或展開器的內容,印出來就消失了。
3. **行長上限(`--copy`)不要套到 `/report/`。** 那一頁是列印文件,同一頁上有滿寬的表格、
   `.kv` 格線與方框;只把段落壓窄會讓大半頁的字只剩一半寬。而且螢幕上的它是**列印預覽**,
   套了上限就會與實際印出來的不一致。其他頁要套也只套自由段落,方框裡的字不要套 ——
   方框有看得見的邊,它自己就是量尺。
