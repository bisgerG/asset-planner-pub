...
{key:'etf',term:'ETF',short:'一檔就買進一籃子股票的基金，可以像股票一樣在盤中買賣。',long:'ETF 是一種「一次買一整籃股票」的基金，掛在交易所，開盤時間隨時能買賣，價格跟著它追蹤的那一籃子股票走。買一張追蹤台灣前 50 大公司的 ETF，等於同時當這 50 家公司的小股東，不必自己挑股票。本站提到的 0050、VT、00631L、SSO 都是 ETF。'},
...
{key:'00631l',term:'00631L',short:'元大台灣50正2，台灣規模最大的 2 倍槓桿 ETF，但追的不是大盤。',long:'00631L 全名「元大台灣50正2」，2014 年掛牌，是台灣同類商品中規模最大的（約 2,224 億）。⚠ 它追的是<b>台灣50</b>，不是「② 挑時機」在看的大盤。過去它純用臺股期貨建倉，交付的實質上就是 2 倍大盤，所以剛好對得上；但<b>2026-05-19 起元大加進台積電現貨</b>去強化追台灣50，官方明講要和 2 倍大盤「做出區隔」—— 詳見「② 挑時機」的風險第 ④ 條。'},
{key:'00685l',term:'00685L',short:'群益臺灣加權正2，「② 挑時機」台股那一份錢用的標的。',long:'00685L 全名「群益臺灣加權正2」，2017 年掛牌，追蹤「臺指日報酬兩倍指數」—— 也就是<b>大盤（加權指數）的每日兩倍</b>，正是「② 挑時機」所有訊號在看的東西。四檔台股正二裡它的內扣費用最低（0.48%），實測的實際成本也最低。⚠ 兩件要注意：它的成交量約是 00631L 的四分之一，而且 2026-07-07 剛做過「1 拆 24」，查歷史股價圖時那天會看起來像暴跌。'},
...
{key:'tqqq_qld',term:'TQQQ／QLD',short:'追蹤那斯達克 100 的 3 倍與 2 倍槓桿 ETF，波動更兇。',long:'QLD 是那斯達克 100 的每日 2 倍、TQQQ 是每日 3 倍槓桿 ETF。本站實測後刻意不採用：那斯達克約七成的漲跌可以直接被 S&P500 解釋，換上它們等於把美股部位再壓一次科技股 —— 那叫加碼，不叫分散。而且 3 倍的那斯達克部位持有 20 年，仍有 41% 的時間區間是虧錢的（不開槓桿是零個），2000 年 3 月網路泡沫頂點進場的 TQQQ 式部位，26 年後到今天仍然是負的。'},
...
{key:'daily_price_limit',term:'單日漲跌幅限制',short:'台股一天有漲跌上限，讓 2 倍 ETF 不可能一天就歸零。',long:'台股個股一天最多漲跌 10%，而槓桿型 ETF 的上限是它的兩倍，也就是 ±20%（00631L 實測最差的一天剛好是 −20.00%）。跌 20% 是剩下 80%，再跌 20% 剩 64%，一直乘下去只會越來越小、永遠除不到 0。這是台股槓桿 ETF 不會被單日崩盤一次清空的原因 —— 但長期被磨光的風險依然存在。'},
...
{key:'backwardation',term:'逆價差',short:'未來月份的期貨比現在便宜，長期持有的人反而佔便宜。',long:'台股的 2 倍 ETF 用期貨來放大部位，而期貨有到期日，每個月得換到下一個月的合約。當下個月的報價比較便宜（這就叫逆價差），換約等於打折續約，過去把 00631L 的持有成本壓到接近零。但 2022 年之後這個折扣反轉成溢價，等於變成倒收你錢。「① 配資金」的預設值由 carry.json 每月自動估算（近 5 年平均）；它會變，要留意。'},
...
/* ── 風險與報酬的刻度 ─────────────────────────────────────────── */
{key:'drawdown',term:'回撤／最大回撤（MDD）',short:'從最近的最高帳面金額算起，現在跌了多少；最大回撤是史上最慘的那一次。',long:'回撤是「從你曾經看過的最高帳面金額，跌到現在少了幾成」。100 萬漲到 120 萬後掉回 90 萬，回撤是從 120 萬算起的 −25%，不是從本金 100 萬算。<b>最大回撤（MDD）</b>則是整段期間最深的那一次 —— 它決定你會不會在半夜恐慌、砍在最低點。舉個真的發生過的數字：「② 挑時機」那套 70/30 做法的歷史最大回撤是 −76.7%，100 萬會一度只剩約 23 萬，之後要漲超過 4 倍才回得到原點。這不是理論假設，是決定要不要照著做之前最該先想清楚的一件事。'},
...
{key:'longest_underwater',term:'最長水下',short:'從跌破前一次高點到重新站回去，最久花了多久。',long:'「水下」是指帳面還沒回到前一次高點的那段日子，最長水下就是歷史上最久的一次。這段期間你每天打開帳戶看到的都是虧損。⚠ 這個數字很吃起點：2000 年起算是 3.6 年，起點拉回 1995 年就變成 7.2 年，1997 那種進場點要 4.7~6.7 年，成本轉差的情境是 10 年。它衡量的是「要忍多久」，不是「會賠多少」。'},
...
{key:'position',term:'部位',short:'你此刻手上實際持有的投資金額或數量。',long:'部位是此刻你帳戶裡真正持有的東西，例如「持有 70 萬元的 00631L」。本站算出你今天該持有多少 ETF、多少現金，拿來跟你實際持有的比對，才知道該不該加碼或減碼。'},
...
{key:'alpha_beta',term:'alpha／beta',short:'beta 是跟著大盤動的部分，alpha 是額外多出來的部分。',long:'beta 說明你的資產跟大盤連動的倍數，beta＝1.4 就是大盤動 1%、你動 1.4%；alpha 則是扣掉連動之後，額外多賺或少賺的部分。實測 00631L 的 beta＝2.04（確實交付兩倍）、alpha 約每年 −0.14%。報酬幾乎全來自放大 beta，所以大盤跌時無處可躲。'},
...
const SURFACE = {
  // 市場與商品
  index:['加權指數'], etf:['ETF'], sp500:['S&P500'], nasdaq:['那斯達克'],
  ...
  leverage:['槓桿'], leverage_2x:['2 倍槓桿','2 倍 ETF','槓桿 ETF','正二'], net_exposure:['淨曝險','放大倍數'],
...
function annotateTerms(root, opt){
  ...
  const SKIP = {SCRIPT:1, STYLE:1, INPUT:1, TEXTAREA:1, OPTION:1, SELECT:1, CODE:1, CANVAS:1,
    H1:1, H2:1, H3:1, H4:1, SUMMARY:1, LABEL:1, TH:1, BUTTON:1};
